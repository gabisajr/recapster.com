...
// define(['i18n', 'tplModalClose', 'fancybox', 'fileupload'], function (__, tplModalClose) {
//
// });


let $modal = $('#apply-job-modal')
...
$('.open-apply-job-modal').fancybox({
  ...
  afterShow: function () {
    $form.find(':input').filter(function () {
      ...
      let filtered = $.inArray(name, ['firstname', 'tel', 'email', 'city', 'message']);
...
$resume.find('input[name="file"]').fileupload({
  ...
  change: function (e, data) {
    let filename = data.files[0].name.trim();
...
$resume.find('.delete').click(function () {
  ...
  let path = $resume.find('input[name="resume"]').val();
