...
// define(['caretEnd', 'fancybox', 'mark-widget', 'autocomplete'], function (caretEnd) {
//
// });

export default function (form, params, callback) {
  if (!form.length) return;
  params = params || {};

  let markWidget = form.find('.mark-widget').markWidget()
  ...
  window.onbeforeunload = function (e) {
    if (text.is(':visible') && text.val().trim()) {
      e = e || window.event;
      let dialogText = __('Вы уверены?'); //todo i18n
